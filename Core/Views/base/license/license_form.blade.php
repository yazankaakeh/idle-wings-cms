...
<head>
    <!-- Page Title -->
    <title>{{ translate('Verify License Key') }}</title>
            ...
            <div class="card justify-content-center auth-card">
                <div class="row justify-content-center">
                    <div class="col-xl-7 col-lg-9">
                        <h4 class="mb-30 font-20">{{ translate('Verify License Key') }}</h4>
                        <form action="{{ route('core.license.key.store') }}" method="post">
                            @csrf
                            @if (count($errors) > 0)
                                <div>
                                    <ul class="p-0">
                            ...
                            <div class="d-flex align-items-center">
                                <button type="submit"
                                    class="btn long mr-20 rounded">{{ translate('Verify Now') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
